...
parser.add_argument('--CADD', help='Either CADD_PHRED or CADD_RAW score will be estimated, default CADD_PHRED',
                    required=False, default="CADD_PHRED")
...
parser.add_argument('--denovo', help='Estimate de novo mutations, WORK IN PROGRESS DO NOT USE', required=False,
                    default="NO")
...
def return_IDs(tfam):
    tfam_csv = pd.read_csv(tfam, sep=' ', names=['POP', 'ID', 'A', 'B', 'C', 'D'])
    IDs = tfam_csv['ID']
    return IDs
...
def extract_columns_cadd(clean_cadd_file):
    ...
    if len(general_CADD) > 0:
        ...
        print("Something is wrong at fun extract_columns_cadd(). Your file has length: " + str(len(clean_cadd_file)))
        print("Stopping program")
        sys.exit()
...
def assign_affected(sharedpos_caddfile):
    ...
    for i in range(0, len(IDs)):
        sharedpos_caddfile.loc[
            sharedpos_caddfile['Allele'] == sharedpos_caddfile[IDs[i] + '_1'], 'RES_' + IDs[i] + '_1'] = 'Affected'
        sharedpos_caddfile.loc[
            sharedpos_caddfile['Allele'] == sharedpos_caddfile[IDs[i] + '_2'], 'RES_' + IDs[i] + '_2'] = 'Affected'
...
def get_summary(cadd_file):
    if len(cadd_file) > 0:
        ...
        short_summary.loc[len(short_summary)] = {'CHR': CHROMOSOME, 'CADD_SUM': CADD_SUM, 'CADD_AVG': CADD_AVG,
                                                 'CADD_MIN': CADD_MIN, 'CADD_MAX': CADD_MAX, 'N': CADD_N}
...
def ind_based_summary(cadd_file):
    ...
    for i in range(0, len(IDs)):
        ...
        RES_tmp = cadd_file[(cadd_file['RES_' + IDs[i] + '_1'] == 'Affected') | (
                    cadd_file['RES_' + IDs[i] + '_2'] == 'Affected')].copy()
        ...
        if len(RES_tmp) > 1:
            ...
            summary_fam.loc[len(summary_fam)] = {'CHR': 'NaN', 'CADD_SUM': 'NaN', 'CADD_AVG': 'NaN', 'CADD_MIN': 'NaN',
                                                 'CADD_MAX': 'NaN', 'N': 'NaN', 'ID': IDs[i]}
...
def main():
    ...
    if DENOVO == 'YES':
        ...
        Final_Summary = pd.concat([summary_fam, summary_general])
